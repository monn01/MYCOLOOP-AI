...
function series(points: Array<{ suhu: number; kelembapan: number; ph: number }>): SensorReadingInput[] {
...
describe("evaluateReadiness", () => {
  ...
  it("mengekspos threshold sesuai PRD (suhu 25-35, ph 6-7, kelembapan 60-65)", () => {
    expect(THRESHOLDS.suhu).toEqual({ min: 25, max: 35 });
    expect(THRESHOLDS.ph).toEqual({ min: 6, max: 7 });
    expect(THRESHOLDS.kelembapan).toEqual({ min: 60, max: 65 });
  });

  it("media belum siap: suhu & kelembapan masih tinggi, pH masih rendah di awal proses", () => {
    const readings = series([
      { suhu: 38, kelembapan: 75, ph: 5.4 },
      { suhu: 37.8, kelembapan: 74.5, ph: 5.5 },
    ]);
  ...
  it("dalam proses: parameter sudah masuk rentang target tapi trennya masih bergerak turun", () => {
    const older = [
      { suhu: 33, kelembapan: 62, ph: 6.5 },
      { suhu: 33.2, kelembapan: 62.1, ph: 6.5 },
      { suhu: 33.1, kelembapan: 61.9, ph: 6.5 },
      { suhu: 32.9, kelembapan: 62, ph: 6.5 },
      { suhu: 33, kelembapan: 62, ph: 6.5 },
    ];
    const recent = [
      { suhu: 30, kelembapan: 62, ph: 6.5 },
      { suhu: 29.8, kelembapan: 62, ph: 6.5 },
      { suhu: 30.1, kelembapan: 62, ph: 6.5 },
      { suhu: 29.9, kelembapan: 62, ph: 6.5 },
      { suhu: 30, kelembapan: 62, ph: 6.5 },
    ];
  ...
  it("anomali: lonjakan suhu mendadak antar pembacaan berurutan", () => {
    const readings = series([
      { suhu: 30, kelembapan: 62, ph: 6.5 },
      { suhu: 30.2, kelembapan: 62, ph: 6.5 },
      { suhu: 38.5, kelembapan: 62, ph: 6.5 }, // lonjakan >6°C
    ]);
  ...
  it("anomali: penurunan pH drastis antar pembacaan berurutan", () => {
    const readings = series([
      { suhu: 30, kelembapan: 62, ph: 6.5 },
      { suhu: 30.1, kelembapan: 62, ph: 6.4 },
      { suhu: 30, kelembapan: 62, ph: 5.0 }, // drop >1.0
    ]);
    ...
    expect(result.status).toBe(ReadinessStatus.BELUM_SIAP);
    expect(result.anomaly).not.toBeNull();
    expect(result.anomaly?.type).toBe("PH_DROP");
  });
