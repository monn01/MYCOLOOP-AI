...
import { ArrowRightIcon, DropletIcon, FlaskIcon, ScaleIcon } from "@/components/ui/icons";

const STAGE = PipelineStage.MIXING;

export default function MixingDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      push({ tone: "safe", title: "Siap Dipindahkan", message: latestDecision.reasoning, autoDismiss: true });
    }
  ...
  if (loading) {
    return <p className="text-sm text-muted-foreground">Memuat status mixing...</p>;
  }
  ...
  if (!batch) {
    return (
      <div className="rounded-card border border-status-neutral-border bg-status-neutral-bg p-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Mulai batch baru untuk memantau proses pencacahan & pencampuran.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      >
        <span className="text-sm font-medium text-card-foreground">Lihat detail Smart Mixing Monitor</span>
