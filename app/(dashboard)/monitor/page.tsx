...
import { AerationControl } from "@/components/dashboard/aeration-control";
import { BATCH_STATUS_STYLE } from "@/lib/ui/status-styles";
import { PRE_CONDITIONING_PARAM_CONFIG } from "@/lib/ui/param-configs";
...
export default function MonitorPage() {
  ...
  if (!batch) {
    return (
      <div className="rounded-card border border-status-neutral-border bg-status-neutral-bg p-6 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">Monitor akan aktif begitu ada batch pre-conditioning.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      <SensorChart data={readings} paramConfig={PRE_CONDITIONING_PARAM_CONFIG} />
