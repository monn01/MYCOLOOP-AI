...
export default function DashboardUtamaPage() {
  ...
  useEffect(() => {
    ...
    if (
      ...
      push({ tone: "safe", title: "Media Siap Disterilisasi", message: latestDecision.reasoning, autoDismiss: true });
    }
  ...
  if (loading) {
    return <p className="text-sm text-muted-foreground">Memuat status chamber...</p>;
  }

  if (!batch) {
    return (
      <div className="rounded-card border border-status-neutral-border bg-status-neutral-bg p-6 text-center">
        <p className="text-sm font-medium text-card-foreground">Tidak ada batch yang sedang berjalan.</p>
        <p className="mt-1 text-sm text-muted-foreground">Mulai batch baru untuk memantau pre-conditioning.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 lg:space-y-8">
      ...
      >
        <span className="text-sm font-medium text-card-foreground">Lihat detail Pre-Conditioning Monitor</span>
