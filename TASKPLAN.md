...
- [x] Model `MixingReading` (pH, kekeruhanAir, beratKg) di `prisma/schema.prisma` — direvisi dari desain awal (kadarAir/rasioCN) lewat migrasi `mixing_actuator_pivot` (2026-07-26, lihat `PRD.md` §1.1); `SensorReading` (Pre-Conditioning) juga di migrasi ini, drop field `pH` (pindah ke Mixing)
- [x] Model baru `ActuatorCommand` (batchId, timestamp, stage, target, action, triggeredBy, reasoning) — audit trail command aktuator, dipakai Mixing (valve) sekarang & Pre-Conditioning (fan) di Phase 5f
- [x] Safety envelope generik `lib/ai/actuatorSafetyEnvelope.ts` (dipakai Mixing sekarang, reusable buat Phase 5f) — sensor sanity-check, fail-safe ke posisi aman, cap durasi buka beruntun (`MAX_CONSECUTIVE_OPEN_COMMANDS`), override manual operator selalu menang; unit test di `lib/ai/actuatorSafetyEnvelope.test.ts`
- [x] Mixing Control Agent `lib/ai/evaluateMixingReadiness.ts` — state machine rule-based (bukan cuma classifier): urutkan buka/tutup valve `limbah_jagung`→`dedak`→`kapur` berdasar berat kumulatif (`MIXING_CHANNELS`, target 100kg placeholder), lalu quality-gate pH (6.0-7.0)/kekeruhan air (200-500 NTU placeholder, kalibrasi Phase 6) dengan koreksi otomatis (reopen kapur/limbah jagung); anomaly deteksi berat turun/pH shift/kekeruhan drop; unit test di `lib/ai/evaluateMixingReadiness.test.ts`
- [x] `lib/sensors/ingestMixing.ts` — jalankan control agent, terapkan safety envelope per channel (diff terhadap command terakhir supaya tidak spam), simpan `ActuatorCommand` yang benar-benar dieksekusi
- [x] Simulator `lib/simulator/curveMixing.ts` + `scripts/simulateMixing.ts` update buat sensor baru (pH naik dari asam ke netral, kekeruhan naik seiring progress), log command valve yang dieksekusi — diverifikasi end-to-end (batch baru → urutan valve limbah jagung→dedak→kapur sesuai berat kumulatif → quality gate)
- [x] API `app/api/mixing-readings/route.ts` (GET, generic) + `app/api/actuator-commands/route.ts` (GET, baru — generic lintas stage) + SSE `app/api/stream/mixing-readings/route.ts`
- [x] Dashboard `app/(dashboard)/mixing/page.tsx` (Metric Card pH/kekeruhan air + info berat) dan `app/(dashboard)/mixing/monitor/page.tsx` (chart + AI Decision Panel + `ValveCommandLog` baru) — diverifikasi visual di browser
- [x] Pre-Conditioning ikut disesuaikan (field `pH` di-drop): `lib/ai/evaluateReadiness.ts`, `lib/sensors/ingest.ts`, `lib/simulator/curve.ts`, `scripts/simulate.ts`, `app/(dashboard)/page.tsx`, `app/(dashboard)/riwayat/[id]/page.tsx`, `lib/ai-assistant/context.ts` dan `lib/ai-assistant/gemini.ts` (system prompt) — diverifikasi visual di browser
- [x] Data demo di `prisma/seed.ts` (batch COMPLETED + RUNNING per stage, plus contoh log `ActuatorCommand` buat batch Mixing selesai)
...
- [ ] Command fan dicatat ke `ActuatorCommand` (model sudah ada dari Phase 5b, lihat `lib/ai/actuatorSafetyEnvelope.ts`) dengan `triggeredBy: AI` atau `MANUAL`
...
## Phase 6a — MQTT Bridge (Software Prep, Selesai)

> Hardware-independent — bagian software dari "aktifkan MQTT bridge" bisa dikerjakan & dites penuh tanpa ESP32 fisik, pakai broker Mosquitto lokal + publisher tiruan. Lihat `PRD.md` §6/§7.5.

- [x] `lib/mqtt/bridge.ts` — real MQTT client (`mqtt` npm package), subscribe ke topic sensor per stage, publish command aktuator ke topic command, resolve/auto-create batch `RUNNING` aktif per stage (sama pola dengan `scripts/simulate*.ts`, firmware tidak perlu tahu `batchId`)
- [x] Kontrak topic (device/stage-scoped, didokumentasikan di komentar `lib/mqtt/bridge.ts` dan `PRD.md` §6): `mycoloop/<stage>/sensor` masuk, `mycoloop/<stage>/command` keluar (Mixing & Pre-Conditioning saja, Incubation tidak punya aktuator)
- [x] Validasi payload (JSON parse + shape check per stage) — payload rusak/tidak sesuai skema di-drop dengan log warning, tidak pernah bikin bridge crash; unit test di `lib/mqtt/bridge.test.ts`
- [x] Proses standalone `scripts/mqttBridge.ts` (`npm run mqtt:bridge`) — dipisah dari Next.js API routes karena butuh koneksi persisten
- [x] "ESP32 palsu" `scripts/mockEsp32.ts` (`npm run mock:esp32 -- --stage <mixing|pre-conditioning|incubation>`) — publish data sensor lewat MQTT sungguhan (bukan insert DB langsung kayak simulator lain), buat nge-tes bridge end-to-end
- [x] Setup Mosquitto lokal via Docker (`docker/mosquitto/mosquitto.conf`, command di README.md) — diverifikasi end-to-end: mock ESP32 → broker → bridge → AI engine → dashboard, ketiga stage, termasuk command valve Mixing terpublish balik dan payload rusak ditangani tanpa crash

## Phase 6 — Integrasi ESP32 & Aktuator (Setelah Hardware Siap)

- [ ] Tulis firmware ESP32 per stage sesuai kontrak topic Phase 6a: Mixing (pH/kekeruhan/berat → `mycoloop/mixing/sensor`, subscribe `mycoloop/mixing/command` buat solenoid valve), Pre-Conditioning (suhu/kelembapan → `mycoloop/pre-conditioning/sensor`, subscribe `mycoloop/pre-conditioning/command` buat fan), Incubation (suhu/kelembapan/CO2/cahaya + capture kamera → `mycoloop/incubation/sensor`/upload gambar)
- [ ] Wiring command aktuator asli: solenoid valve (Mixing) dan fan (Pre-Conditioning) — safety envelope dari Phase 5b/5f harus aktif sebelum firmware menerima command produksi
- [ ] Arahkan `MQTT_BROKER_URL` di `.env` produksi ke broker asli (Mosquitto self-hosted/HiveMQ Cloud, bukan Mosquitto lokal Phase 6a) dan jalankan `npm run mqtt:bridge` di server — namespace topic per stage tetap dipakai walau 1 ESP32 gabungan atau 3 papan terpisah (keputusan tim hardware, lihat `PRD.md` §6)
...
**Phase 5b dan 6a** (Mixing sensor+actuator pivot, MQTT bridge) sudah selesai. **Phase 5f dan 6b** (baru, dari pivot arsitektur 2026-07-26) sama-sama hardware-independent — bisa dikerjakan pakai simulator/Mosquitto lokal kapan saja, sama seperti Phase 5e dan 8, tanpa nunggu Phase 6. Ketiganya (5f, 6a, 6b) independen satu sama lain, bisa dikerjakan dalam urutan apa saja atau paralel; 5f tinggal pakai model `ActuatorCommand`/`lib/ai/actuatorSafetyEnvelope.ts` yang sudah ada dari Phase 5b. Phase 6 (hardware asli) sekarang jauh lebih kecil scope-nya karena bridge-nya sudah jadi & teruji (Phase 6a) — tinggal firmware ESP32 (di luar scope Claude Code, tim hardware) publish/subscribe ke kontrak topic yang sudah didokumentasikan, arahkan `MQTT_BROKER_URL` ke broker produksi, dan wiring aktuator asli — safety envelope dari Phase 5b/5f wajib aktif dulu sebelum itu.
